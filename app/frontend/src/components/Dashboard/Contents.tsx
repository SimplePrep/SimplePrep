...
const Contents: React.FC<ContentsProps> = ({ isDarkMode }) => {
  ...
  const modeClass = isDarkMode ? 'text-white' : 'text-gray-800';
    ...
    return (
      <div className="my-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {tests.map((test, index) => {
          ...
          return (
            <div key={test.id} className={`p-6 rounded-lg border-2 ${borderColorClass} shadow-lg ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-800'}`}>
              <div className="flex justify-between items-center mb-6">
                <div className="text-xl text-purple-500">{test.id}</div>
              </div>
              <h5 className="text-xl leading-tight font-medium mb-2">{test.title}</h5>
  ...
  return (
    <div className={`max-w-[1200px] h-full mx-auto ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
      <div className="py-40 rounded-2xl">
        <p className={`p-10 font-medium text-xl ${modeClass}`}>
          Freemium English and Writing Practice tests
        </p>
