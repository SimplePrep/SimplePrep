              ...
              <div className='p-14'>
                ...
                <div className='flex flex-col mt-7 gap-2'>
                  {answerChoices.map((choice, index) => {
                    const isCorrect = choice.label === currentQuestion.correct_answer;
                    const isSelected = userAnswer?.selectedChoice === choice.label;
                    return (
                      <button
                        key={index}
                        className={`py-2 px-4 border-2 rounded-lg font-semibold text-lg w-full text-left
                          ${isSelected ? (isCorrect ? 'border-green-500' : 'border-red-500') : 'hover:border-blue-500'}
