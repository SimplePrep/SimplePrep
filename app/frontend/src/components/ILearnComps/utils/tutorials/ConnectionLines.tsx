...
const MixCard: React.FC<MixCardProps> = ({ isDarkMode, isComplete, isActive, position, verticalPosition, index, section }) => {
    const horizontalPosition = position === 'left' ? '0%' : position === 'right' ? 'calc(100% - 320px)' : '50%';
    const transform = position === 'middle' ? 'translateX(-50%)' : 'translateX(0)';
    ...
    return (
      <div 
        className={`MixCard w-72 md:w-80 max-w-sm border-4 ${isComplete ? (isDarkMode ? 'border-green-600' : 'border-green-500') : (isDarkMode ? 'border-blue-600' : 'border-blue-500')} rounded-3xl absolute`} 
        style={{
      ...
      >
        <div className={`m-2 flex flex-col gap-5 md:gap-10 ${isDarkMode ? 'bg-[#1d263b]' : 'bg-white'} rounded-xl shadow-md`}>
          ...
          <button
                className={`m-3 py-3 px-4 rounded-xl border-[1px] transition-colors duration-200 ${isComplete ? 'hover:bg-green-600 hover:text-white' : ''} ${isDarkMode ? 'border-slate-400 text-white hover:bg-gray-700' : 'border-gray-300 text-gray-900 hover:bg-gray-200'} font-semibold rotate-on-hover`}
                >
                {isComplete ? (
                    <div className='flex flex-row gap-2 items-center justify-center'>
                    <p>Review</p>
                    <VscDebugRestart size={25} className="icon-rotate" />
                    </div>
                ) : (
                    <p>Start</p>
                )}
