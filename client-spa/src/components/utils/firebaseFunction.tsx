...
exports.registerUserInBackendAndCleanup = functions.firestore
    ...
    .onUpdate(async (change: functions.Change<functions.firestore.DocumentSnapshot>, context: functions.EventContext) => {
        ...
        if (!beforeData?.emailVerified && afterData?.emailVerified) {
            const userData = {
                ...
                firebase_uid: context.params.userId,
            };

            try {
                const response = await axios.post('http://localhost:8080/auth/user/signup', userData);
                console.log('Backend registration successful:', response.data);

                if (response.data.success) {
                    await admin.firestore().doc(`users/${context.params.userId}`).delete();
